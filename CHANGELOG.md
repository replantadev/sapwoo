...
- **Recuperación con cliente autorizado** - el reintento operativo puede utilizar un CardCode explícito para un único pedido, después de comprobar en SAP que corresponde a un cliente activo. La sustitución existe solo durante esa petición y no modifica la vinculación del usuario Woo.
...
- **Plantilla generica** - renombra la plantilla y la herramienta de inspeccion Crystal, elimina assets particulares y conserva un fallback al logo general del sitio cuando Care no esta disponible.
- **Asset de actualizacion determinista** - si un release contiene ZIP duplicados, el actualizador selecciona expresamente `sap-woo-suite-vX.Y.Z.zip` en lugar de depender del orden devuelto por GitHub.
...
- **Maquetacion de factura** - el escritor reproduce cabecera, cliente, lineas, impuestos, vencimientos, totales y pagina legal en A4 con primitivas PDF estandar.
...
- **Endpoint `POST /control/mark-exported`** — marca un pedido como exportado manualmente desde el Control Center. Establece `_sap_exported=1` y `_sap_no_sync=1` para excluir el pedido de futuros intentos de sincronización, y limpia los metas de reintento (`_sap_retry_exhausted`, `_sap_retry_count`, `_sap_export_error`, `_sap_sync_failed_at`). Para pedidos creados manualmente en SAP B1. Requiere autenticación X-SAPWC-Secret + IP del CC en whitelist.
