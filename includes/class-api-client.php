...
class SAPWC_API_Client {
    ...
    public function login($user, $pass, $db) {
        ...
        if ($code === 200 && isset($data['SessionId'])) {
            ...
            // Asegurarse de extraer solo el valor de ROUTEID
    ...
    public function get($relative_path) {
        ...
        //$this->log_response('GET ' . $relative_path, $response); // 👈 aquí
    ...
    // Método para obtener información de la versión:
    public function get_version_info() {
        $response = $this->get('/Service_GetVersion');
    
        if (isset($response['ServiceLayerVersion'])) {
            return [
                'ServiceLayerVersion' => $response['ServiceLayerVersion'],
                'SAPB1Version'        => $response['SAPB1Version'] ?? '–',
                'CompanyDB'           => $response['CompanyDB'] ?? '–',
